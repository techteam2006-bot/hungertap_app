...
import { getCachedProfile, setCachedProfile } from './settingsCache';
...
export function AuthProvider({ children }) {
  ...
  async function syncProfileAndRole(id) {
    ...
    // Soft hydrate from AsyncStorage while network sync runs.
    try {
  ...
  /**
   * Signup OTP — check auth.users first so we never send mail for an existing email.
   * @param {string} email
  ...
  async function sendSignupEmailOtp(email, metadata = {}) {
    ...
    try {
      ...
      // Pre-check BEFORE signInWithOtp — avoids sending a code when the email is taken.
      try {
