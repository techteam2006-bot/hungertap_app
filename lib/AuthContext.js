...
export function AuthProvider({ children }) {
        ...
        (async () => {
          ...
          if (error) {
            ...
            const { data: created, error: refetchErr } = await supabase
              ...
              .maybeSingle();
            if (refetchErr) {
              console.warn('syncProfile refetch:', refetchErr.message || refetchErr);
              setProfile(null);
              setUserRole(null);
            } else if (created) {
              setProfile(created);
              const role = created.role || null;
              setUserRole(role);
              if (role && role !== 'student') {
                await handleSignOut();
                setAuthError('Access denied. This app is only for students.');
  ...
  async function signInWithPassword(email, password) {
    ...
    try {
      ...
      if (error) {
        ...
        let enriched = { ...error, message: msg };
        if (
          ...
          try {
            const { data: alreadyExists, error: existsRpcError } = await supabase.rpc(
            ...
            if (!existsRpcError && rpcAuthEmailExistsResultIsTrue(alreadyExists)) {
              enriched = {
                ...error,
                code: 'incomplete_signup_or_wrong_password',
                message: msg,
                accountExists: true,
              };
            }
  ...
  /**
   * Signup: reject if email already registered, then send OTP (creates auth user on verify).
   * @param {string} email
  ...
  async function sendSignupEmailOtp(email, metadata = {}) {
    ...
    try {
      ...
      if (existsRpcError && __DEV__) {
        console.warn('check_auth_email_exists failed (OTP send will still be attempted):', existsRpcError.message);
      }

      const userData = buildAuthSignUpUserData(metadata);
      beginPendingSignup(metadata);

      // Email already in auth (often OTP-verified with no password after a failed Sign Up).
      // Resend OTP so they can finish and set a password — a hard block left login broken.
      const resumeExisting = !existsRpcError && rpcAuthEmailExistsResultIsTrue(alreadyExists);
      const result = await sendEmailOtp(normalizedEmail, {
        shouldCreateUser: !resumeExisting,
        data: userData,
      });
      if (result.error) {
        clearPendingSignup();
      }
      return resumeExisting && !result.error ? { ...result, resumed: true } : result;
    } catch (e) {
      clearPendingSignup();
      setAuthError(e.message || String(e));
      return { data: null, error: { message: e.message || String(e) } };
