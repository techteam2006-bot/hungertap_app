  ...
  async function completePasswordReset(newPassword) {
    try {
      ...
      if (updateError) {
        ...
        const mapped = mapSignupError(updateError);
        if (mapped.type === 'password_rejected' || mapped.type === 'invalid_password') {
          return {
            success: false,
        ...
        return {
          success: false,
          error: mapped.message || 'Could not update your password. Please try again.',
          errorType: mapped.type,
        };
      }
