...
export const canteenStatusService = {
  ...
  async checkCanteenStatus(canteenId = null, userId = null) {
    try {
      ...
      // Prefer student-readable view; never fall back to `canteens`
      // (RLS/grants often yield "permission denied for table canteens").
      ...
      if (!viewRes.error && viewRes.data) {
        ...
        error = viewRes.error;
        // Still try takeaway via SECURITY DEFINER RPC so cart fee can sync.
        try {
          ...
          if (!rpcRes.error && row?.takeaway_charge != null) {
            rememberLastCanteen(canteenId, row.name || '', {
              takeawayCharge: row.takeaway_charge,
            }).catch(() => {});
          }
        } catch (_) {
          // ignore — status path already failed
        }
      }
