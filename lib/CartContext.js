...
import { getLastRememberedCanteen } from './menuCache';
import { loadTakeawayFeeForUser, isUsableCachedTakeawayFee } from './takeawayFee';
...
export const CartProvider = ({ children }) => {
  ...
  /** Per non-beverage unit from canteens.takeaway_charge; null until loaded (no fake ₹10). */
  const [takeawayFeePerItem, setTakeawayFeePerItemState] = useState(null);
  const [takeawayFeeLoading, setTakeawayFeeLoading] = useState(true);
  const { user } = useAuth();
  const { showAppAlert } = useAppAlert();
  const showAppAlertRef = useRef(showAppAlert);
  showAppAlertRef.current = showAppAlert;
  ...
  const alertOrderLimit = () => {
    showAppAlertRef.current('Order limit', CART_MAX_ORDER_TOTAL_MESSAGE);
  };
  ...
  const setIsTakeaway = (enabled) => {
    ...
    if (next) {
      const fee = takeawayFeeRef.current;
      if (fee == null) {
        showAppAlertRef.current(
          'Takeaway',
          'Takeaway charge is still loading. Please wait a moment and try again.'
        );
        return false;
      }
      if (wouldExceedMaxOrderTotal(cartItemsRef.current, true, fee)) {
        alertOrderLimit();
        return false;
      }
    }
  ...
  // Sync takeaway fee from cache + live canteens.takeaway_charge (retries on poor network).
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setTakeawayFeeLoading(true);
      try {
        const last = await getLastRememberedCanteen();
        if (!cancelled && isUsableCachedTakeawayFee(last?.takeawayCharge)) {
          setTakeawayFeePerItem(last.takeawayCharge);
        }
        const loaded = await loadTakeawayFeeForUser(user?.id, { retries: 3 });
        if (cancelled) return;
        if (loaded.fee != null) {
          setTakeawayFeePerItem(loaded.fee);
        }
      } catch (_) {
      } finally {
        if (!cancelled) setTakeawayFeeLoading(false);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const refreshTakeawayFee = useCallback(async () => {
    setTakeawayFeeLoading(true);
    try {
      const loaded = await loadTakeawayFeeForUser(user?.id, { retries: 3 });
      if (loaded.fee != null) {
      ...
      return takeawayFeeRef.current;
    } finally {
      setTakeawayFeeLoading(false);
    }
  }, [user?.id]);
  ...
  const addToCart = async (item) => {
    ...
    setCartItems((prevCartItems) => {
      ...
      if (!check.ok) {
        ...
        showAppAlertRef.current(title, check.error);
