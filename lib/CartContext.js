...
export const CartProvider = ({ children }) => {
  ...
  const setIsTakeaway = (enabled) => {
    ...
    if (next && wouldExceedMaxOrderTotal(cartItemsRef.current, true)) {
      alertOrderLimit();
      return false;
    }
  ...
  const replaceCartItems = (lines) => {
    ...
    if (wouldExceedMaxOrderTotal(next, isTakeawayRef.current)) {
      alertOrderLimit();
      return false;
    }
    setCartItems(next);
  ...
  const addToCart = async (item) => {
    ...
    setCartItems((prevCartItems) => {
      ...
      if (wouldExceedMaxOrderTotal(nextCart, isTakeawayRef.current)) {
        alertOrderLimit();
        return prevCartItems;
      }
  ...
  const getPayableTotal = (takeawayOverride) => {
    ...
    return cartPayableTotal(cartItems, takeaway);
  };
