...
import { loadLocalCart, clearLocalCartStorage } from './localCartStorage';
import { supabase } from './supabase';

/** Remote Supabase `cart_items` — table is not deployed on live backend; default is local. */
...
class CartTimeoutService {
  ...
  async setupNotificationChannels() {
    try {
      if (Constants.appOwnership === 'expo') {
        console.log('📱 Expo Go: skipping cart notification channel setup');
        return;
      }
  ...
  async fetchCartItemsForMonitoring(userId) {
    if (!REMOTE_CART) {
      const localItems = await loadLocalCart(userId);
      return (Array.isArray(localItems) ? localItems : []).map((item, index) => ({
        ...item,
        created_at: item.created_at || item.added_at || new Date(0).toISOString(),
        id: item.cartItemId ?? item.id ?? `local-${index}`,
      }));
    }
  ...
  async startCartMonitoring(userId) {
    ...
    try {
      ...
      const cartItems = await this.fetchCartItemsForMonitoring(userId);
  ...
  async clearExpiredCart(userId) {
    try {
      if (!REMOTE_CART) {
        await clearLocalCartStorage(userId);
      } else {
