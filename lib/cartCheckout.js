...
import { maxOrderableQtyForItem } from './cartRules';
import {
...
export async function prepareCheckoutCart(userId, cartLines) {
  const empty = { cartAfterCanteen: [], removedWrongCanteen: [], stockErrors: [] };
  ...
  try {
    ...
    return { cartAfterCanteen, removedWrongCanteen, stockErrors };
  } catch (err) {
