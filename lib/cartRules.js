...
/** Max payable total (items + takeaway). Soft/hard gate in cart checkout. */
...
export const CART_MAX_ORDER_TOTAL_MESSAGE = `Orders cannot exceed ₹${CART_MAX_ORDER_TOTAL}. Remove items or turn off takeaway to continue.`;
 ...
 */
export function validateAddToCart(prevLines, item, addQtyRaw) {
  if (!item?.id) {
 ...
 * @param {string} itemId
 */
export function validateIncreaseQuantity(prevLines, itemId) {
