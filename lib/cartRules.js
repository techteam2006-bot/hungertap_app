...
/**
 * Normalize a known per-unit takeaway fee from canteen row / AsyncStorage.
 * Returns null when unknown — callers must not invent ₹10 for display.
 * Never treat null/undefined/'' as ₹0 (`Number(null) === 0`).
 * @param {unknown} fee
 * @returns {number|null}
 */
...
/**
 * Fee to use in money math when takeaway is on.
 * Unknown fee → 0 charge (do not invent a fallback amount).
 */
export function effectiveTakeawayFeePerItem(fee) {
  const n = resolveTakeawayFeePerItem(fee);
  return n == null ? 0 : n;
}
...
export function takeawayChargeForLines(lines, isTakeaway = false, feePerItem) {
  ...
  const fee = effectiveTakeawayFeePerItem(feePerItem);
  if (fee <= 0) return 0;
