...
function friendlyCheckoutError(data, httpStatus) {
  ...
  if (data && typeof data === 'object') {
    ...
    if (typeof err === 'string' && err.trim().length > 0) {
      ...
      if (err === 'no_active_default_gateway_configured') return 'No active payment gateway is currently configured.';
 ...
 * Easebuzz and Razorpay WebView fallbacks use it when the native SDK is unavailable.
 */
...
/** Razorpay Key ID (public) returned by create-order-v2. */
export function extractRazorpayKeyIdFromV2Body(body) {
  if (!body || typeof body !== 'object') return '';
  const v = body.key_id ?? body.keyId ?? body.data?.key_id;
  return v != null ? String(v).trim() : '';
}

/** Razorpay order id (`order_…`) stamped on payments.gateway_order_id. */
export function extractRazorpayOrderIdFromV2Body(body) {
  if (!body || typeof body !== 'object') return '';
  const v =
    body.razorpay_order_id ??
    body.razorpayOrderId ??
    body.gateway_response?.id ??
    body.data?.razorpay_order_id;
  return v != null ? String(v).trim() : '';
}

/** Amount in paise from Razorpay create-order response. */
export function extractRazorpayAmountPaiseFromV2Body(body) {
  if (!body || typeof body !== 'object') return '';
  const v = body.amount ?? body.gateway_response?.amount ?? body.data?.amount;
  if (v == null || v === '') return '';
  const n = Number(v);
...
export async function postCreateOrderV2(args) {
  ...
  try {
    ...
    return {
      ...
      key_id: extractRazorpayKeyIdFromV2Body(data),
      razorpay_order_id: extractRazorpayOrderIdFromV2Body(data),
      amount_paise: extractRazorpayAmountPaiseFromV2Body(data),
      status: res.status,
      data,
    };
  } catch (e) {
...
export async function navigateToPaymentProcessingAfterV2(navigation, supabaseClient, v2, ctx) {
  ...
  if (gateway === 'razorpay') {
    const keyId = v2.key_id || extractRazorpayKeyIdFromV2Body(v2.data);
    const razorpayOrderId =
      v2.razorpay_order_id || extractRazorpayOrderIdFromV2Body(v2.data);
    const amountPaise = v2.amount_paise || extractRazorpayAmountPaiseFromV2Body(v2.data);
    if (!keyId || !razorpayOrderId || !amountPaise) {
      return {
        ok: false,
        error: 'Razorpay checkout could not start: order details were not returned from the server.',
      };
    }
    ...
    navigation.navigate('PaymentProcessing', {
      ...
      paymentUrl: v2.payment_url || extractPaymentUrlFromV2Body(v2.data) || '',
    });
    return { ok: true };
  }
