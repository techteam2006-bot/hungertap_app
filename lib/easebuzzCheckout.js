...
function parseSdkPayload(raw) {
  ...
  try {
    ...
    if (lower.includes('payment_success') || lower.includes('success') || lower.includes('txn_success')) {
      return { result: 'payment_success', status: 'success', raw: s };
    }
    if (
      lower.includes('user_cancelled') ||
      lower.includes('cancelled') ||
      lower.includes('back_press') ||
      lower.includes('dropped')
    ) {
      return { result: 'user_cancelled', status: 'cancelled', raw: s };
    }
    if (lower.includes('payment_failed') || lower.includes('failed') || lower.includes('failure') || lower.includes('error')) {
      return { result: 'payment_failed', status: 'failure', raw: s };
    }
    return { result: s, raw: s };
  }
}
