...
export async function getMenu(canteenId = null, options = {}) {
  ...
  // Edge `get-canteen-menu` requires canteen_id — never hit the network as `_all`.
  if (id === UNSCOPED_ID) {
    cacheLog('CACHE MISS', { canteenId: id, reason: 'no_canteen_id', forceRefresh });
    return {
      data: [],
      error: { message: 'canteen_id is required', code: 'MENU_NO_CANTEEN' },
      fromCache: false,
