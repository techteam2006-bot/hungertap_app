 ...
 * @param {string} [name]
 */
export async function rememberLastCanteen(canteenId, name = '') {
  const id = normalizeCanteenId(canteenId);
  if (id === UNSCOPED_ID) return;
  try {
    await AsyncStorage.setItem(
      LAST_CANTEEN_KEY,
      JSON.stringify({ id, name: name || '', at: Date.now() })
    );
  } catch (_) {}
}
...
export async function getLastRememberedCanteen() {
  try {
    ...
    if (parsed?.id != null && String(parsed.id)) {
      return { id: String(parsed.id), name: typeof parsed.name === 'string' ? parsed.name : '' };
    }
  } catch (_) {}
  return null;
}
