/**
 * Map money fields from `order_items` / `archived_order_items` (names vary by migration).
 */
const num = (v) => {
...
const LINE_SUBTOTAL_KEYS = [
  ...
  'total_price',
  'item_total',
  /** `archived_order_items` snapshots often store the line bill as `total_amount` */
  'total_amount',
  'amount',
  'snapshot_line_total',
  'final_amount',
...
/**
 * Normalize a live/archived/failed line row for Order Status bill UI.
 * Prefer snapshotted `line_total` so catalog renames cannot blank the price.
 */
export function mapLineRowToBillItem(oi) {
  const name = String(oi?.item_name || oi?.items?.name || 'Item').trim();
  const quantity = Number(oi?.quantity ?? 1) || 1;
  const lineTotal = getLineSubtotalFromRow(oi);
  const unit = getUnitPriceForRow(oi);
  const catalogPrice =
    oi?.items?.price != null && Number.isFinite(Number(oi.items.price))
      ? Number(oi.items.price)
      : undefined;
  const money = lineTotal >= 0.01 ? Number(lineTotal) : undefined;
  return {
    name: name || 'Item',
    quantity,
    line_total: money,
