...
async function fetchLinesForOrder(supabase, table, orderId) {
  try {
    ...
    if (table === ARCHIVED_LINE_ITEMS_TABLE) {
      const fb = await fetchArchivedOrderItemsByOrderIds(supabase, [orderId]);
      return fb;
    }
    if (table === FAILED_LINE_ITEMS_TABLE) {
      const fb = await fetchFailedOrderItemsByOrderIds(supabase, [orderId]);
      return fb;
    }
...
export async function fetchUserOrdersWithLineJoins(supabase, userId) {
  try {
    ...
    if (liveRes.error) {
      console.error('fetchUserOrdersWithLineJoins live:', liveRes.error.message || liveRes.error);
    ...
    if (liveLinesRes.error && liveIds.length) {
      console.error('order_items batch:', liveLinesRes.error.message || liveLinesRes.error);
      return { data: [], error: liveLinesRes.error };
    }
