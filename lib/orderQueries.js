 ...
 *
 * Never join archived_order_items to live `orders` (IDs are gone after close).
 */
import {
...
export async function fetchUserOrdersPage(supabase, userId, page = {}) {
  try {
    ...
    for (const row of headers) {
      ...
      if (src === 'archived') archIds.push(id);
...
export async function fetchOrderWithLineJoins(supabase, orderId, userId) {
  try {
    ...
    // 2) Post-close archive — header PK is `id` (original order id); no `order_id` column
    ...
    if (!archHeader.error && archHeader.data) {
      const normalized = normalizeHistoryOrder(archHeader.data, 'archived');
    ...
    // 3) Failed payment history — same header shape as archived_orders
