...
export const ORDER_STATUS_FILTERS = [
  ...
  { id: 'cancelled_by_vendor', label: 'Cancelled by vendor' },
...
export function getOrderStatusNotificationBody(status, itemSummary = '') {
  ...
  switch (status) {
    ...
    case 'cancelled_by_vendor':
      return items
        ? `Order cancelled by vendor: ${items}. A refund will be processed.`
        : 'Your order was cancelled by the vendor. A refund will be processed.';
    case 'payment_cancelled':
      return items ? `Payment cancelled: ${items}` : 'Payment was cancelled for this order';
    default:
...
export function orderMatchesStatusFilter(orderStatus, filterId) {
  ...
  if (filterId === 'cancelled_by_vendor') {
    return isCancelledLike(orderStatus) && orderStatus !== 'payment_cancelled';
  }
  if (filterId === 'payment_failed') {
    return isPaymentFailedLike(orderStatus) || orderStatus === 'payment_cancelled';
  }
