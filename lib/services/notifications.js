...
/**
 * Persist native FCM / device push token on `user_tokens.fcm_token` (keyed by `user_id`).
 * Column is plain `text` — see Supabase `user_tokens` table.
...
async function savePushTokenToSupabase(userId, tokenValue) {
  ...
  const payload = { fcm_token: tokenValue, is_enabled: true };
