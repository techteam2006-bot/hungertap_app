...
// Prefer CONFIG (single source) so URL/key stay consistent with Edge menu helpers.
const SUPABASE_URL = CONFIG.SUPABASE_URL || process.env.EXPO_PUBLIC_SUPABASE_URL || '';
const SUPABASE_ANON_KEY =
  CONFIG.SUPABASE_ANON_KEY || process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY || '';

/**
 * SecureStore adapter for Supabase auth — stores JWT tokens in Android Keystore / iOS Keychain.
 * Always mirrors to AsyncStorage so offline cold-starts can restore the session even if
 * SecureStore is unavailable or the session JSON exceeds SecureStore size limits.
 */
const secureStoreAdapter = {
  ...
  async setItem(key, value) {
    ...
    // Always persist to AsyncStorage for offline restore.
    try {
...
function buildSupabaseClient() {
  ...
  try {
    ...
    // Use warn (not error) so RN LogBox does not cover the UI with a red toast.
    if (!url || !key) {
      console.warn(
        '[Supabase] Missing EXPO_PUBLIC_SUPABASE_URL or EXPO_PUBLIC_SUPABASE_ANON_KEY; copy .env.example to .env and restart Expo.'
      );
    }
...
export const checkDatabaseTables = async () => {
  try {
    ...
    const tables = ['users', 'items', 'categories', 'orders', 'order_items', 'canteens', 'colleges'];
  const results = {};
...
export const foodService = {
   ...
   * @param {string|null} canteenId - When set, only rows from `items` for this canteen (must match `users.canteen_id` for checkout).
   */
  async getFoodItems(categoryId = null, canteenId = null) {
    try {
      ...
      const cid = canteenId != null && String(canteenId).trim() !== '' ? String(canteenId).trim() : '';
      if (!cid) {
        // Edge/HTTP menu APIs require canteen_id — callers must wait until canteen is known.
        return { data: [], error: { message: 'canteen_id is required', code: 'MENU_NO_CANTEEN' } };
      }

      if (menuFromHttpEnabled()) {
        const { data, error } = await getHttpMenuItemsFiltered(categoryId, cid, false);
      ...
      const { data, error } = await getEdgeMenuItemsFiltered(categoryId, cid, false);
      if (!error && Array.isArray(data)) {
        const normalizedData = data.map((item) => normalizeFoodItemNutrition({ ...item }));
  ...
  async getFoodItemsPage(categoryId = null, canteenId = null, offset = 0, pageSize = 10, pageOptions = {}) {
    ...
    if (!cid) {
      return {
        data: [],
        error: { message: 'canteen_id is required', code: 'MENU_NO_CANTEEN' },
        hasMore: false,
      };
    }
    ...
    try {
      if (menuFromHttpEnabled()) {
        const {
          ...
          error,
          hasMore,
        } = await getHttpMenuItemsPage(categoryId, cid, offset, ps, forceRefresh);
      ...
      const {
        ...
        error,
        hasMore,
      } = await getEdgeMenuItemsPage(categoryId, cid, offset, ps, forceRefresh);
      if (!error && Array.isArray(rows)) {
        const normalized = rows.map((item) => normalizeFoodItemNutrition({ ...item }));
        return { data: normalized, error: null, hasMore: !!hasMore };
      }
...
/** Matches `create_order_v2` / Edge checkout line limits (c_max_lines / c_max_quantity_per_line). */
...
/**
 * Validates cart lines for Edge `create-order-v2` / RPC `create_order_v2`.
 ...
 */
export function prepareCheckoutOrderArgs(linesSource, isTakeawayInput) {
...
/**
 * Before checkout, ensure each line exists in `public.items` for `users.canteen_id`.
 ...
 * @param {Array<{ item_id: string, quantity: number }>} pItems from prepareCheckoutOrderArgs
...
/**
 * Maps order RPC / Edge raise messages to short, user-facing text.
 * @param {string|null|undefined} message
 * @returns {string}
 */
export function formatCheckoutOrderError(message) {
...
/** @deprecated Use prepareCheckoutOrderArgs */
export function prepareCreateOrderMinimalArgs(linesSource, isTakeawayInput) {
  return prepareCheckoutOrderArgs(linesSource, isTakeawayInput);
}

/** @deprecated Use formatCheckoutOrderError */
export function formatCreateOrderMinimalRpcError(message) {
  return formatCheckoutOrderError(message);
}

/** @deprecated Use prepareCheckoutOrderArgs — returns [] on failure (avoid). */
export function buildCreateOrderMinimalItemsPayload(cartItems) {
  const r = prepareCheckoutOrderArgs(cartItems, false);
...
/**
 * Optional legacy worker: POSTs to `CONFIG.ORDER_API_BASE/order` when `ORDER_HTTP_ENABLED`.
 * App checkout uses Edge `create-order-v2` (`postCreateOrderV2`) — do not call this for paid student orders.
 *
  ...
  try {
    if (!CONFIG.ORDER_HTTP_ENABLED) {
      return {
        ...
        error: {
          message:
            'Legacy order HTTP API is disabled. Use Edge create-order-v2 checkout.',
          code: 'ORDER_HTTP_DISABLED',
        },
      };
    }
...
// Favorites: device-local only (no `favorites` table on live backend).
export const favoritesService = {
  async getFavorites(userId) {
    try {
      if (!userId) return { data: [], error: null };
      const data = await loadLocalFavorites(userId);
      return { data, error: null };
    } catch (error) {
  ...
  async addToFavorites(userId, foodItemId, itemSnapshot = null) {
    try {
      if (!userId || foodItemId == null) {
        return { data: null, error: 'User and item are required.' };
      }
      const data = await addLocalFavorite(userId, itemSnapshot, foodItemId);
      return { data, error: null };
    } catch (error) {
  ...
  async removeFromFavorites(userId, foodItemId) {
    try {
      ...
      await removeLocalFavorite(userId, foodItemId);
      return { error: null };
    } catch (error) {
  ...
  async isFavorite(userId, foodItemId) {
    try {
      ...
      const isFavorite = await isLocalFavorite(userId, foodItemId);
  ...
  async toggleFavorite(userId, foodItemId, itemSnapshot = null) {
    try {
      if (!userId || foodItemId == null) {
        return { data: null, error: 'User and item are required.', removed: false };
      }
      ...
      const { data, error } = await this.addToFavorites(userId, foodItemId, itemSnapshot);
      if (error) {
        return { data: null, error, removed: false };
      }
...
/**
 * Food availability Realtime disabled — app only uses OrderStatusScreen websockets.
 * Kept as a no-op so older callers do not crash.
 */
export const subscribeToFoodAvailability = async (_callback, _options = {}) => {
  return null;
};
