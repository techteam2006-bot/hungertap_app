...
export const foodService = {
  ...
  async getFoodItems(categoryId = null, canteenId = null) {
    try {
      ...
      // Availability: `available_stock` (0 = out of stock); null stock = not limited in UI
      let query = supabase
        .from('items')
        .select(`
          *,
      ...
      if (result.error) {
        console.error('❌ Error fetching food items:', result.error);
        ...
        if (canteenId) fallbackQuery = fallbackQuery.eq('canteen_id', canteenId);
        ...
        if (fallbackResult.error) {
          ...
          console.log('✅ Fallback query successful:', fallbackResult.data?.length, 'items');
          // Add default values for missing fields
          const enrichedData = fallbackResult.data
            ?.map(item => ({
              ...item,
              description: 'Item',
            ...
            ?.map(normalizeFoodItemNutrition);
          
          return { ...fallbackResult, data: enrichedData };
        }
      ...
      if (Array.isArray(result.data)) {
        result.data = result.data.map(item => normalizeFoodItemNutrition({
          ...item,
          image_url: item.image_url ?? null,
          is_vegetarian: item.is_vegetarian || false,
          available_stock: item.available_stock ?? null,
      ...
      console.log('✅ Food items fetched successfully:', result.data?.length, 'items');
      return result;
  ...
  /**
   * Paginated menu fetch — same schema as {@link #getFoodItems}, with `.range()` for infinite scroll.
   * @param {string|null} categoryId
   * @param {string|null} canteenId
   * @param {number} offset
   * @param {number} pageSize
   * @param {{ forceHttpRefresh?: boolean }} [pageOptions]
  ...
  async getFoodItemsPage(categoryId = null, canteenId = null, offset = 0, pageSize = 10, pageOptions = {}) {
    const forceHttpRefresh = Boolean(pageOptions?.forceHttpRefresh);
    ...
    try {
      if (menuFromHttpEnabled()) {
        const {
          ...
          error,
          hasMore,
        } = await getHttpMenuItemsPage(categoryId, canteenId, offset, ps, forceHttpRefresh);
      ...
      const result = await query.order('name', { ascending: true }).range(from, to);

      if (result.error) {
        console.warn('getFoodItemsPage primary:', result.error.message || result.error);
        let fallbackQuery = supabase
          .from('items')
          .select(
            'id, name, price, category_id, is_active, canteen_id, image_url, is_vegetarian, available_stock',
            { count: 'exact' }
          );
        if (canteenId) fallbackQuery = fallbackQuery.eq('canteen_id', canteenId);
        if (categoryId) fallbackQuery = fallbackQuery.eq('category_id', categoryId);

        const fallbackResult = await fallbackQuery.order('name', { ascending: true }).range(from, to);
        ...
        const fr = fallbackResult;
        const fc = typeof fr.count === 'number' ? fr.count : null;
        const hasMore =
          fc !== null ? from + enrichedData.length < fc : enrichedData.length >= ps;

        return { data: enrichedData, error: null, hasMore };
      }

      const rows = Array.isArray(result.data) ? result.data : [];
      const normalized = rows.map((item) =>
        normalizeFoodItemNutrition({
          ...item,
          image_url: item.image_url ?? null,
          is_vegetarian: item.is_vegetarian || false,
          available_stock: item.available_stock ?? null,
      ...
      const hasMore =
        total !== null ? from + normalized.length < total : normalized.length >= ps;

      return { data: normalized, error: null, hasMore };
    } catch (error) {
