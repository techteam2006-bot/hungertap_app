...
/** HMAC-SHA256 hex — signed Razorpay WebView checkout tokens. */
...
/** Token for hosted razorpay-checkout page: base64(order:payment:exp).hmac */
async function buildRazorpayCheckoutToken(
  orderId: string,
  paymentId: string,
  secret: string,
  ttlSeconds = 3600
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (finalGateway === "razorpay") {
      ...
      let rzPaymentUrl = "";
      if (supabaseUrl && keySecret) {
        try {
          const token = await buildRazorpayCheckoutToken(String(order_id), String(payment_id), keySecret);
          rzPaymentUrl = `${supabaseUrl}/functions/v1/razorpay-checkout?token=${encodeURIComponent(token)}`;
        } catch (tokenErr) {
          console.warn("[create-order-v2] Razorpay checkout token failed:", tokenErr);
        }
      }
